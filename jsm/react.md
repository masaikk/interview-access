...
export const TextureSelector = () => {
  ...
  const [activateTexture] = useStore((state) => [state.texture]);
  ...
  return visible && <div>TextureSelector</div>;
};

```

值得注意的是这里的setTimeout操作是在两秒之后设置能见度为false。然后在useEffect的回调函数中消除这个狗子。之后需要添加选择cube的函数
