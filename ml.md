...
仅作为书本内容笔记，其他机器学习知识笔记在mlearning文件夹下面。
...
+ Boosting
  ...
  训练分类器的时候使用串行的方式，各分类器之间互有依赖。

  基本思路是将基分类器层层叠加，每一层在训练的时候，对前面一层的基分类器分错的样本，给予更高的权重。测试时，各层分类器的结果加权得到最终结果。

+ Bagging

  在bagging里面，各基分类器之间无强依赖，可以进行并行训练。在最终的时候，每个个体单独做出判断，再投票做出最后的集体决策。

  好处在于集成后的分类器方差，比基分类器方差小。



## 生成对抗网络 

训练判别器，是在度量生成器分布和真实数据分布JS距离。训练生成器，是在减小这个JS距离。
